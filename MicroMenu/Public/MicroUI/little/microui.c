...
/************************************* 旋钮相关 *************************************/
// 按键ID
#define BTN_ID_CC 0 // 逆时针旋转-->上一页
#define BTN_ID_CW 1 // 顺时针旋转-->下一页
...
void list_draw_value(int n)
{
    ...
    // u8g2_DrawStr(&u8g2, CHECK_BOX_L_S, list.temp + LIST_TEXT_H + LIST_TEXT_S, check_box.v[n - 1]);
}
...
void window_show()
{
    ...
    u8g2_DrawRFrame(&u8g2, win.l + 5, (int16_t)win.y + 20, WIN_BAR_W, WIN_BAR_H, 1);
    u8g2_DrawBox(&u8g2, win.l + 7, (int16_t)win.y + 22, win.bar, WIN_BAR_H - 4);
    u8g2_DrawStr(&u8g2, win.l + 5, (int16_t)win.y + 14, (char *)win.title);
    u8g2_DrawStr(&u8g2, win.l + 78, (int16_t)win.y + 14, (char *)win.value);
